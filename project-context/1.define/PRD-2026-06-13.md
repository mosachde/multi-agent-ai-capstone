...
**LLM configuration (MVP — Decision D11):**

| Setting | Value |
|---------|-------|
| Provider | **Anthropic** (`ANTHROPIC_API_KEY` via `.env`) |
| Model | **`claude-sonnet-4-20250514`** for `manager_llm` and all specialist agents |
| Temperature | **0.2** (deterministic artifact generation) |
| Token budget | Per-workflow cap documented in SAD; target **< USD 2.00/run** (Section 7) |

Single-provider stack for MVP simplicity; mixed-provider routing deferred to Phase 2 if cost/latency tuning requires it.
...
- **Acceptance criteria:**
  ...
  - CRM draft JSON uses **HubSpot standard field mapping (Decision D10):**
    - Deal: `dealstage`, `hs_next_step`, `closedate`
    - Activity: engagement note body (HubSpot `NOTE` type)
    - References: `associatedContactId`, `associatedDealId` from fixture or live CRM (Phase 2)
- **Agent:** follow_up
- **MRD trace:** P0 #7
...
**D10 — HubSpot CRM draft schema defaults (confirmed 2026-06-13).**

- **Decision:** CRM draft card (FR-P0-07) and synthetic fixtures use **standard HubSpot deal and activity fields**:

| Output | HubSpot property / object | MVP notes |
|--------|---------------------------|-----------|
| Deal stage suggestion | `dealstage` | Enum aligned to fixture pipeline stages |
| Next step | `hs_next_step` | Free text |
| Close date suggestion | `closedate` | ISO date string |
| Activity log | Engagement **Note** body | No auto-create in MVP; copy/export only |
| Contact link | `associatedContactId` | From synthetic fixture |
| Deal link | `associatedDealId` | From synthetic fixture |

- **Rationale:** Standard properties map 1:1 to HubSpot UI labels reps already know; minimizes Phase 2 integration rework.
- **Cascading implications:** Synthetic fixture dataset and CRM draft UI card must expose these fields; SAD defines JSON schema for agent output validation.

**D11 — Anthropic Claude for all MVP agents (confirmed 2026-06-13).**

- **Decision:** **`claude-sonnet-4-20250514`** via Anthropic API for `manager_llm` and every specialist agent. Single provider in MVP.
- **Rationale:** One API key, one billing stream, consistent behavior across hierarchical crew; avoids mixed-provider debugging during Phase 1 demo.
- **Cascading implications:** `.env.example` documents `ANTHROPIC_API_KEY`; SAD records model ID, temperature (0.2), and per-workflow token budgets; OpenAI/Azure deferred unless Phase 2 cost review warrants change.
...
11. **CRM draft schema:** HubSpot standard fields per Decision D10 (`dealstage`, `hs_next_step`, `closedate`, note body).
12. **LLM stack:** Anthropic `claude-sonnet-4-20250514` for all agents per Decision D11.
...
4. ~~Minimum HubSpot fields for CRM draft schema?~~ **Resolved — see Decision D10 (HubSpot standard: `dealstage`, `hs_next_step`, `closedate`, note body).**
...
6. ~~LLM provider preference (OpenAI vs. Anthropic)?~~ **Resolved — see Decision D11 (Anthropic Claude for all MVP agents).**
...
**All open questions resolved.** PRD ready for `@system.arch` handoff (Phase 1 MVP scope).
...
| Field | Value |
|-------|-------|
| Timestamp | 2026-06-13T14:00:00-05:00 |
| Persona | @product-mgr |
| Action | … → update-v1.8-session-ttl-microsoft-phase2-synthetic-crm → rename PRD-2026-06-13 → update-v1.9-resolve-open-questions-d10-d11 |
...
| Traceability | MRD-v2 → PRD → **ready for SAD** (@system.arch, Phase 1 MVP) |
